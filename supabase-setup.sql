...
CREATE INDEX idx_subscriptions_payfast_id ON subscriptions(payfast_subscription_id);
...
CREATE INDEX idx_ai_jobs_service ON ai_jobs(service);
...
CREATE INDEX idx_crm_listings_user_id ON crm_listings(user_id);
CREATE INDEX idx_crm_listings_status ON crm_listings(status);
CREATE INDEX idx_crm_listings_contact_id ON crm_listings(contact_id);
CREATE INDEX idx_media_items_user_id ON media_items(user_id);
CREATE INDEX idx_media_items_listing_id ON media_items(listing_id);
...
-- Users policies - can only access own data
...
-- Admin audit log - only admins can access
