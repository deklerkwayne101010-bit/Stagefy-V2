...
export function AgentShowcaseWizard({
  ...
  const handleGenerate = () => {
    ...
    if (designStyle === 'business_card') {
      ...
      prompt = `Create an agent personal branding image with large prominent tagline "${tagline}" as the focus. Include small agent photo in corner, name "${agentProfile?.name || 'Agent'}" in elegant typography below tagline. Use ${orientation} orientation. Use ${mainColor} as the main background color, ${secondaryColor} for text/elements, and ${accentColor} for highlights/accents following 60/30/10 design rule. Agent contact info: Phone: ${agentProfile?.phone || 'Phone'}, Email: ${agentProfile?.email || 'Email'}. IMPORTANT: This is a QUOTE-FOCUSED design - do NOT include any property photos or property images in the design - only agent branding elements (agent photo, name, tagline, contact details).`
