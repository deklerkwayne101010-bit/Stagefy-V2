...
export default function VideoStudioPage() {
  ...
  const totalDur = useMemo(()=>computeTotalDur(clips,txDurs,proj.transitions,vidRef.current),[clips.map(c=>c.url).join('|'),txDurs.join('|'),proj.transitions.map(t=>t.duration).join('|')])
  ...
  const clipDursRef=useRef(clipDurs);clipDursRef.current=clipDurs
  const txDursRef=useRef(txDurs);txDursRef.current=txDurs
  const transitionsRef=useRef(proj.transitions);transitionsRef.current=proj.transitions
  const brandingRef=useRef(proj.branding);brandingRef.current=proj.branding
  ...
  const renderFrame=useCallback((time:number)=>{
    ...
    if(i>=0&&i<clips.length){const clip=clips[i];const src=clip.url;const isV=src.startsWith("blob:")||/\.(mp4|webm|mov|avi)(\?.*)?$/i.test(src);
      if(isV){if(!vidRef.current){vidRef.current=document.createElement("video");vidRef.current.crossOrigin="anonymous";vidRef.current.muted=true;vidRef.current.preload="auto";vidRef.current.setAttribute("playsinline","")}const v=vidRef.current;if(v.src!==src){v.src=src;v.load();const sync=()=>{if(v.duration&&isFinite(v.duration)&&v.duration>0){setProj(s=>({...s,clips:s.clips.map(c=>c.url===src?{...c,trimEnd:v.duration,duration:v.duration}:c)}))}};v.addEventListener('loadedmetadata',sync,{once:true});v.addEventListener('durationchange',sync,{once:true})}if(playing){v.play().catch(()=>{})}const vw=v.videoWidth||w,vh=v.videoHeight||h;const sc=Math.max(w/Math.max(vw,1),h/Math.max(vh,1));const dw=vw*sc,dh=vh*sc;ctx.drawImage(v,(w-dw)/2,(h-dh)/2,dw,dh)}
  ...
  const handleFiles=useCallback(async(files:FileList|File[])=>{setUploading(true);const nc:VideoClip[]=[];for(const f of Array.from(files)){if(!f.type.startsWith("image/")&&!f.type.startsWith("video/"))continue;const url=URL.createObjectURL(f);const d=f.type.startsWith("video/")?60:5;nc.push({id:uid(),url,name:f.name,duration:d,trimStart:0,trimEnd:d,sortOrder:proj.clips.length+nc.length})}setProj(p=>({...p,clips:[...p.clips,...nc]}));setUploading(false)},[proj.clips.length])
  ...
  const startExport=useCallback(async()=>{if(clips.length===0||exporting)return;setExporting(true);setExportProg("Preparing...");setShowExport(true)
  ...
  useEffect(()=>{try{const s=localStorage.getItem("vsProj");if(s){const p=JSON.parse(s);const migrated={...p,clips:(p.clips||[]).map((c:any)=>{const isV=(c.url||'').startsWith("blob:")||/\.(mp4|webm|mov|avi)(\?.*)?$/i.test(c.url||'');if(!isV)return c;const d=c.trimEnd-c.trimStart;return d<3?{...c,trimEnd:60,duration:60}:c})};setProj(v=>({...v,...migrated,branding:migrated.branding??v.branding,audio:migrated.audio??v.audio}))}}catch{}},[])
  ...
  useEffect(()=>{const v=vidRef.current;if(v){const update=()=>{if(v.duration&&isFinite(v.duration)&&v.duration>0){setProj(s=>({...s,clips:s.clips.map(c=>c.url===v.src?{...c,trimEnd:v.duration,duration:v.duration}:c)}))}};v.addEventListener('loadedmetadata',update);v.addEventListener('durationchange',update);return()=>{v.removeEventListener('loadedmetadata',update);v.removeEventListener('durationchange',update)}}},[clips.map(c=>c.url).join('|')])
  return (
    <>
