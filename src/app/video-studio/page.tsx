...
export default function VideoStudioPage() {
  ...
  const renderFrame = useCallback((time:number)=>{
    ...
    if(i>=0&&i<clips.length){const clip=clips[i];const src=clip.url;const isV=src.startsWith("blob:")||/\.(mp4|webm|mov|avi)(\?.*)?$/i.test(src);
      if(isV){if(!vidRef.current){vidRef.current=document.createElement("video");vidRef.current.crossOrigin="anonymous";vidRef.current.muted=true;vidRef.current.preload="auto";vidRef.current.setAttribute("playsinline","")}const v=vidRef.current;if(v.src!==src)v.src=src;if(playing){const seek=clip.trimStart+Math.max(0,lt-0.05);if(v.readyState>=2){if(Math.abs(v.currentTime-seek)>0.5)v.currentTime=seek;if(v.paused)v.play().catch(()=>{})}}const vw=v.videoWidth||w,vh=v.videoHeight||h;const sc=Math.max(w/Math.max(vw,1),h/Math.max(vh,1));const dw=vw*sc,dh=vh*sc;ctx.drawImage(v,(w-dw)/2,(h-dh)/2,dw,dh)}
