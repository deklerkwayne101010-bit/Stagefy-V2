...
const ASPECT=[{value:'16:9',label:'Landscape 16:9'},{value:'9:16',label:'Portrait 9:16'}] as const
const TXTYPES=[{value:'fade',label:'Crossfade'},{value:'fade-black',label:'Fade-Black'},{value:'slide',label:'Slide'},{value:'zoom',label:'Zoom'},{value:'none',label:'None'}] as const
const DTX=1.0
...
async init(){if(!this.c){this.c=new AudioContext();this.mg=this.c.createGain();this.pg=this.c.createGain();this.mg.gain.value=0.4;this.mg.connect(this.c.destination);this.pg.connect(this.mg)}}
...
const renderFrame = useCallback(async(time:number)=>{
  ...
  const resize=useCallback(()=>{const c=mainRef.current;if(!c)return;const p=c.parentElement;if(!p)return;const s=aspectSz(proj.aspectRatio,Math.min(960,p.clientWidth-8));c.width=s.w;c.height=s.h},[proj.aspectRatio]);
  ...
  const stop=useCallback(()=>{playOff.current=0;setPlayTime(0);audioRef.current?.stop();setPlaying(false);if(vidRef.current){vidRef.current.pause();vidRef.current.src=""}}(),[]);
  ...
  const startExport=useCallback(async()=>{
    ...
    setExporting(true);setExportProg('Preparing...');setShowExport(true);
    ...
    if(!audioRef.current)audioRef.current=new MusicEngine();
    await audioRef.current.init();
    audioRef.current.setMusicVolume(proj.audio.musicVolume);
    audioRef.current.setMasterVolume(proj.audio.masterVolume);
    audioRef.current.start();
    const stream=cvs.captureStream(30);
    try{const a=audioRef.current as any;if(a.ctx){const dest=a.ctx.createMediaStreamDestination();if(a.pg)a.pg.connect(dest);dest.stream.getAudioTracks().forEach((t:any)=>stream.addTrack(t))}}catch{}
    ...
    <div className="flex h-[100svh] flex-col bg-[#0b0f19] text-slate-200 overflow-hidden">
      ...
      <div className="flex flex-1 min-h-0">
        <aside className="w-80 border-r border-slate-800 bg-[#0b0f19] overflow-y-auto p-3 space-y-4 shrink-0">
          <Card>
            <CardHeader title="Listing Photos" subtitle="Images or video clips" />
          ...
          <Card>
            <CardHeader title="Branding Card" subtitle="Agent overlay on every frame" />
            <div className="space-y-2">
              ...
              {proj.branding.logoDataUrl && (
                <div className="flex items-center gap-2 mt-2">
                  <img src={proj.branding.logoDataUrl} alt="logo" className="w-10 h-10 rounded border border-slate-700 object-contain bg-slate-900" />
                  <button onClick={()=>setBranding({logoDataUrl:null})} className="text-xs text-red-400 hover:text-red-300">✕ Remove</button>
                </div>
              )}
            </div>
          </Card>
          ...
          <Card>
            <CardHeader title="Audio Mixer" subtitle="Procedural soundtrack" />
        ...
        <aside className="w-80 border-l border-slate-800 bg-[#0b0f19] overflow-y-auto p-3 space-y-4 shrink-0">
          <Card>
            <CardHeader title="Timeline" subtitle={`${clips.length} clip(s)`} />
          ...
          <Card>
            <CardHeader title="Transitions" subtitle="Between clips" />
      ...
      {showExport && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center">
          <div className="bg-[#141926] border border-slate-700 rounded-xl p-6 shadow-2xl max-w-sm w-full">
            ...
            <div className="w-full bg-slate-800 rounded-full h-2 mb-4"><div className="bg-emerald-500 h-2 rounded-full transition-all" style={{width:exportProg.includes('%')?exportProg.match(/\\d+%/)![0]:'0%'}} /></div>
